...
public abstract class AbstractSpecimenController {
	...
	public Collection<LabResult> getLabResults(@RequestParam(required = false, value = "patientId") Integer patientId) {
		
		Patient patient = Context.getPatientService().getPatient(patientId);
		
		return Context.getService(TbService.class).getLabResults(patient);
		
	}
	
	@ModelAttribute("status")
	public String getStatus(@RequestParam(required = false, value = "patientId") Integer patientId) {
		
		Patient patient = Context.getPatientService().getPatient(patientId);
		
		String pi= Context.getAdministrationService().getGlobalProperty("mdrtb.primaryPatientIdentifierType");
		if(patient.getPatientIdentifier(pi) == null)
			return "Suspect";
		else
			return "Confirmed";
	...
	@ModelAttribute("labResult")
	public LabResult getLabResult(@RequestParam(required = false, value = "patientId") Integer patientId,
									@RequestParam(required = false, value = "labResultId") Integer labResultId) {
		
		if(labResultId == null)
			return null;
		
		return Context.getService(TbService.class).getLabResult(labResultId);
